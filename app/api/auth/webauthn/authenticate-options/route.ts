...
export async function POST(req: Request) {
  try {
    ...
    if (!opts || !opts.challenge) {
      ...
      return NextResponse.json({ ok: false, message: "challenge generation failed" }, { status: 500 });
    }

    // store challenge
    await prisma.user.update({ where: { id: user.id }, data: { verifyToken: String(opts.challenge) } });

    // return options directly; ensure allowCredentials ids are strings
    const jsonSafe = { ...opts };
    if (Array.isArray(jsonSafe.allowCredentials)) jsonSafe.allowCredentials = jsonSafe.allowCredentials.map((c: any) => ({ ...c, id: String(c.id) }));
