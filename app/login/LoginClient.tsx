...
export default function LoginClient() {
  ...
  const [msg, setMsg] = useState<string | null>(registered ? "登録完了しました。ログインしてください。" : null);
          ...
          {step === "select" ? (
            <div className="flex flex-col gap-3">
              ...
              <p className="text-xs text-center text-muted mt-2">
                アカウントをお持ちでない方は
                <Link className="underline ml-1 text-primary" href="/register">こちらから登録</Link>
              </p>
            </div>
          ) : step === "email" ? (
            <form onSubmit={handlePasswordLogin} className="flex flex-col gap-3">
              ...
              {msg && <div className="text-sm text-red-600" role="status">{msg}</div>}
            ...
            <form onSubmit={handleVerifyOtp} className="flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                <motion.button
                  type="button"
                  className="w-1/2 bg-white dark:bg-gray-800 rounded-full border dark:border-gray-600 py-2 text-sm dark:text-gray-100"
                ...
                <button
                  type="button"
                  className="w-1/2 text-sm underline"
                  onClick={() => setStep("email")}
                  disabled={loading}
                >
                  ← 戻る
